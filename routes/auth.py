...
def login():
    ...
    access_token = create_access_token(identity=user.id,additional_claims={"email":user.email},expires_delta=timedelta(days=90))
    response.headers["x-auth-tokne"] = access_token
